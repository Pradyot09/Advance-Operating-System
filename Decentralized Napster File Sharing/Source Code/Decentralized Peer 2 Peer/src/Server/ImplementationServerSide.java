...
public class ImplementationServerSide implements Runnable
{
	private Socket clientSocket;
	public static ConcurrentHashMap<String, ArrayList<String>> DHT = new ConcurrentHashMap<String, ArrayList<String>>();
	private boolean loop = true;
	
	//Make a Change of File Path here for the download Method
        
	public static String FilePath = "C:\\Users\\prady\\OneDrive\\Documents\\NetBeansProjects\\Decentralized Peer 2 Peer\\p2pSharedFolder\\";
        
	public ImplementationServerSide(Socket clientSocket) 
	{
			...
			while(loop)
			{
				try
				{
					...
					case "2":	
						...
						try
						{
							List<String> value = SearchFile(key);	//results where file is present
					...
					case "3":	//Perform OBTAIN Operation
						...
						FileDownload(fileName,clientSocket);
						break;
	...
	/*
	 * to search for Filename & its location(s) 
	 */
	public static List<String> SearchFile(String fileName) throws IOException
	{
		...
		filePeers = DHT.get(fileName);	//get the Value(s) i.e location, for the Key(Filename)

		return filePeers;
	}

	/*
	 * to register the files requested by the client
	...
	public static synchronized boolean reigstry(String key, String value)
	{
		...
		check = DHT.get(key);	//get the value(PeerID) for the Key(Filename)

		//check if file is already registered by the same peer, if not register it
		if(check == null || check.isEmpty())
		{
			DHT.put(fileName, peers);
	...
	/*
	 * to download file, requested by the Client, to its specified location
	 */
	public static void FileDownload(String fileName, Socket clientSocket)
	{
		...
		try
		{
			...
			File checkFile = new File(FilePath + peerName +"/" + fileName);

			//creating intput streams & buffer streams for reading
			FileInputStream fin = new FileInputStream(checkFile);
			BufferedInputStream bir = new BufferedInputStream(fin);
			...
			while((numOfRead = bir.read(buffContent)) != -1)
			{
